...
$log = '/var/log/agents_log';
$buffer = 100;
$hostname = gethostname();
$table_format = '%s_agents';

$day = date('z');
$date = date('Ymd');
$table = sprintf($table_format, $date);
$skeleton = 'skel_agents';
...
function writeBuffer($values) {
	...
	$query = "INSERT INTO {$table} (host, pid, vhost, rhost, date, agent) VALUES {$values}";
...
$statsDb = @new mysqli($host, $user, $pass, $db);
if ($statsDb->connect_error) {
	doLog("Failed to connect to MySQL on {$host}: {$statsDb->connect_error}");
}
...
while ($hit = trim(fgets(STDIN))) {
	...
	if (count($hit) != 7) {
		array_splice($hit, 6, count($hit), implode(' ', array_slice($hit, 6)));
	}

	if (count($hit) == 7) {
		$hit[5] = str_split($hit[5], 3);
		$hit[6] = addslashes($hit[6]);

		$values[] = "('{$hostname}', '{$hit[0]}', '{$hit[1]}', INET_ATON('{$hit[2]}'), CONVERT_TZ('{$hit[3]} {$hit[4]}', '{$hit[5][0]}:{$hit[5][1]}', 'America/Denver'), '{$hit[6]}')";

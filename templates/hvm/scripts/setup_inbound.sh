...
if [ -z "${master}" -o -z "${server_id}" -o -z "${offset}" -o -z "${bucket_name}" ]; then
        echo "Usage: ${BASH_SOURCE[0]} -m master_name:master_ip -i server_id -o offset -b files_bucket_name [-h hostname_prefix] [-e environment_suffix]"
        exit 1
fi
...
cp "/${filename}" "/${filename}.orig"
sed -i -r \
-e "\|^cluster\s+\{$|,\|^\}$|s|(\s+name\s+\=\s+)\".*\"|\1\"Inbound\"|" \
